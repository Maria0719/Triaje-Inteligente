...
export default function PatientCard({ patient, onMarkAttended }: PatientCardProps) {
    ...
    return (<div className={cn("bg-card rounded-xl border border-border border-l-4 p-4 transition-all hover:shadow-md hover:shadow-primary/5 cursor-pointer", mtsColorMap[patient.mtsLevel])}>
      ...
      <div className="flex items-center justify-between">
        <span className={cn("flex items-center gap-1 text-sm font-medium", waitExceeded ? "text-destructive" : "text-muted-foreground")}>
        ...
        <div className="flex gap-2">
          <button onClick={(e) => { e.stopPropagation(); navigate(`/patients/${patient.id}`); }} className="flex items-center gap-1 text-xs font-medium text-primary border border-primary/30 rounded-lg px-3 py-1.5 hover:bg-primary/5 transition-colors">
            <Eye className="h-3.5 w-3.5"/> Ver detalle
          </button>
          <button onClick={(e) => { e.stopPropagation(); onMarkAttended?.(patient.id); }} className="flex items-center gap-1 text-xs font-medium text-primary-foreground bg-mts-4 rounded-lg px-3 py-1.5 hover:opacity-90 transition-colors">

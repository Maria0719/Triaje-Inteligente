  ...
  const { loadPatients } = useApp();
    ...
    const handleClassify = async () => {
      ...
      try {
        ...
        let persistedPatientId = incomingPatient?.id;
        try {
          const createdPatient = await patientApiService.createPatient({
            firstName,
            lastName,
          ...
          persistedPatientId = createdPatient.id;
        }
        catch (error) {
          console.error('Failed to persist patient in backend:', error);
        }
      ...
      <div className="p-6 max-w-3xl mx-auto w-full">
        ...
        <div className="flex items-center gap-2 mb-8">
          ...
          {steps.map((s, i) => (<div key={s} className="flex items-center gap-2 flex-1">
              ...
              <div className={cn("h-8 w-8 rounded-full flex items-center justify-center text-sm font-semibold shrink-0", step > i + 1 ? "bg-mts-4 text-primary-foreground" : step === i + 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>
        ...
        <div className="bg-card rounded-xl border border-border p-6">
          ...
          {step === 1 && (<div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  {(['M', 'F', 'Otro'] as Sex[]).map(s => (<button key={s} onClick={() => setSex(s)} className={cn("flex-1 py-2 rounded-lg text-sm font-medium border transition-colors", sex === s ? "bg-primary text-primary-foreground border-primary" : "border-input text-muted-foreground hover:bg-muted")}>
          ...
          {step === 2 && (<div className="space-y-5">
              ...
              <div>
                ...
                <textarea value={complaint} onChange={e => setComplaint(e.target.value)} rows={3} className="w-full rounded-lg border border-input bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring" placeholder="Describa el motivo principal de consulta..."/>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {symptomOptions.map(s => (<button key={s} onClick={() => toggleItem(symptoms, setSymptoms, s)} className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-colors", symptoms.includes(s) ? "bg-primary text-primary-foreground border-primary" : "border-input text-muted-foreground hover:bg-muted")}>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {historyOptions.map(h => (<button key={h} onClick={() => toggleItem(history, setHistory, h)} className={cn("px-3 py-1.5 rounded-full text-xs font-medium border transition-colors", history.includes(h) ? "bg-primary text-primary-foreground border-primary" : "border-input text-muted-foreground hover:bg-muted")}>
              ...
              <div>
                ...
                <input type="range" min={1} max={10} value={painScale} onChange={e => setPainScale(Number(e.target.value))} className="w-full accent-primary"/>
          ...
          {step === 3 && (<div className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  ...
                  {avpuOptions.map(a => (<button key={a} onClick={() => setAvpu(a)} className={cn("py-2.5 rounded-lg text-sm font-medium border transition-colors", avpu === a ? "bg-primary text-primary-foreground border-primary" : "border-input text-muted-foreground hover:bg-muted")}>
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < 3 ? (<Button onClick={() => setStep(step + 1)} className="bg-primary text-primary-foreground">

                Siguiente <ArrowRight className="h-4 w-4 ml-1"/>

              </Button>) : (<Button onClick={handleClassify} disabled={loading} className="bg-primary text-primary-foreground px-6">

...
export default function PatientDetailPage() {
  ...
  const [markError, setMarkError] = useState('');
    ...
    const handleMarkAttended = async () => {
      if (!id) {
        setMarkError('No se pudo identificar el paciente.');
        return;
      }

      setMarkError('');
      try {
        await patientApiService.updatePatientStatus(id, 'attended');
        setPatients(prev => prev.map(p => p.id === id ? { ...p, status: 'attended' as const } : p));
        navigate('/dashboard');
      }
      catch (error) {
        console.error('Failed to persist patient attended status:', error);
        setMarkError('No se pudo marcar como atendido. Intenta nuevamente.');
      }
    };
    return (<AppLayout>
      ...
      <div className="p-6 max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="bg-card rounded-xl border border-border p-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 h-2 bg-muted rounded-full relative overflow-hidden">
              ...
              <div className={cn("h-full rounded-full transition-all", patient.waitTimeMinutes > config.maxWait ? "bg-destructive" : "bg-mts-4")} style={{ width: `${Math.min((patient.waitTimeMinutes / Math.max(config.maxWait, 1)) * 100, 100)}%` }}/>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          {vitals.map(v => {
            ...
            return (<div key={v.key} className={cn("bg-card rounded-xl border p-4", normal ? "border-mts-4/30" : "border-destructive/30")}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-card rounded-xl border border-border p-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {patient.symptoms.map(s => (<span key={s} className="bg-secondary text-secondary-foreground text-xs px-2.5 py-1 rounded-full">{s}</span>))}
          ...
          <div className="bg-card rounded-xl border border-border p-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {patient.medicalHistory.map(h => (<span key={h} className="bg-secondary text-secondary-foreground text-xs px-2.5 py-1 rounded-full">{h}</span>))}
        ...
        {patientAlerts.length > 0 && (<div className="bg-card rounded-xl border border-border p-4">

            <h3 className="text-sm font-semibold mb-3">Historial de alertas</h3>

            <div className="space-y-2">

              {patientAlerts.map(a => (<div key={a.id} className="text-sm text-muted-foreground border-l-2 border-destructive/40 pl-3 py-1">
        ...
        <div className="flex gap-3">
          ...
          <Button variant="outline" className="flex-1" onClick={() => navigate('/patients/new', { state: { patient, isUpdate: true, returnTo: `/patients/${patient.id}` } })}>

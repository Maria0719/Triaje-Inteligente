...
export default function DashboardPage() {
  const { patients, setPatients, alerts, setAlerts, userName } = useApp();
  ...
  if (filter === 'critical') filtered = filtered.filter(p => p.mtsLevel <= 2);
  else if (filter === 'urgent') filtered = filtered.filter(p => p.mtsLevel === 3);
  else if (filter === 'mild') filtered = filtered.filter(p => p.mtsLevel >= 4);

  if (search) {
    const s = search.toLowerCase();
    filtered = filtered.filter(p =>
      `${p.firstName} ${p.lastName}`.toLowerCase().includes(s) || p.id.toLowerCase().includes(s)
    );
  }
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              {filtered.length === 0 ? (
                ...
                filtered.map(p => (
                  <PatientCard key={p.id} patient={p} onMarkAttended={handleMarkAttended} />
                ))
              )}
            </div>
          </div>
              ...
              {alerts.filter(a => !a.read).length === 0 ? (
                ...
                alerts.filter(a => !a.read).map(alert => (
                  <div key={alert.id} className={cn(
                    "border-l-4 rounded-lg p-3 bg-muted/30",
                    alert.severity === 1 ? "border-l-mts-1" : alert.severity === 2 ? "border-l-mts-2" : alert.severity === 3 ? "border-l-mts-3" : "border-l-mts-4"
                  )}>
